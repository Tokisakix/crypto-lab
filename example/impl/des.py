...
def encode_des_block(block: List[int], subkeys: List[List[int]]) -> List[int]:
    ...
    for i in range(16):
        ...
        xor_result = [x ^ y for x, y in zip(expanded_right, subkeys[i])]

        sbox_result = []
        for j in range(8):
            bits = xor_result[j * 6:(j + 1) * 6]
            row = (bits[0] << 1) + bits[5]
            col = (bits[1] << 3) + (bits[2] << 2) + (bits[3] << 1) + bits[4]
            sbox_value = S_BOX[j][row][col]
            sbox_result.extend([int(b) for b in format(sbox_value, '04b')])

        p_result = permute(sbox_result, P)

        new_right = [x ^ y for x, y in zip(left, p_result)]
        left, right = new_left, new_right

    cipher_block = permute(right + left, IP_INV)
    return cipher_block

def encode_des(message: str, key: str) -> str:
    ...
    for i in range(0, len(message_bits), 64):
        ...
        cipher_block = encode_des_block(block, subkeys)
        ciphertext.extend(cipher_block)
    ciphertext_hex = ''.join(format(b, '01b') for b in ciphertext)
    encode = ' '.join([hex(int(ciphertext_hex[i:i + 8], 2)).upper()[2:] for i in range(0, len(ciphertext_hex), 8)])
